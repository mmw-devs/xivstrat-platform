...
导入识别要求：根对象含 `phases` 数组及 `metadata` 对象，后者必须含字符串类型的 `id`、`name`、`title`（允许空字符串）；`references`、`macros` 若存在必须为数组。仅有 `phases: []` 或空 metadata 的 JSON 会被拒绝，解析失败不会替换当前编辑状态。旧版无版本号/v1 导出继续支持；这是最低格式识别，不替代提交前业务校验。
